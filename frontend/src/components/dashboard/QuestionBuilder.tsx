    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="space-y-6">
        {questions.map((q, index) => (
          <div key={q.id || index} className="group relative border rounded-lg bg-white shadow-sm hover:shadow transition-all flex flex-col">
            <div className="flex items-center justify-between p-3 border-b bg-slate-50/50 rounded-t-lg">
              <div className="flex items-center gap-3 w-full">
                ...
                <select 
                  className="text-xs p-1 border rounded bg-white text-slate-700 font-medium"
                  value={q.type}
              ...
              <div className="flex-1 space-y-4">
                <div>
                  <textarea
                    ...
                    className="w-full text-sm font-medium text-slate-800 p-3 border rounded-md focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 outline-none resize-y min-h-[80px]"
                    value={q.content}
                ...
                {q.type !== "Tự luận" && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                    {q.options.map((opt, i) => (
                      <div key={i} className={`flex items-center gap-3 p-2 rounded-md border text-sm transition-colors ${opt.isCorrect ? 'bg-emerald-50 border-emerald-300' : 'bg-slate-50'}`}>
                        <input 
                          type="radio" 
                          name={`correct_${index}`} 
                        ...
                        />
                        <div className="w-6 h-6 rounded border border-slate-300 bg-white flex items-center justify-center text-[10px] font-bold text-slate-500 shrink-0">
