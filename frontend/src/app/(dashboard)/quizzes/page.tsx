...
export default function QuizzesPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case 'published': return <span className="px-2 py-1 bg-emerald-100 text-emerald-700 rounded-md text-xs font-medium">Đang Mở</span>;
      case 'draft': return <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded-md text-xs font-medium">Bản Nháp</span>;
      case 'closed': return <span className="px-2 py-1 bg-rose-100 text-rose-700 rounded-md text-xs font-medium">Đã Đóng</span>;
      default: return null;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Danh sách Đề thi</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        ...
        <div className="p-4 border-b bg-slate-50/50 flex flex-col sm:flex-row gap-4 justify-between items-center">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button variant="outline" className="gap-2 bg-white flex-1 sm:flex-none">
              <Filter className="w-4 h-4 text-slate-500" /> Trạng thái
            </Button>
            <Button variant="outline" className="gap-2 bg-white flex-1 sm:flex-none">
              <Filter className="w-4 h-4 text-slate-500" /> Môn học
            </Button>
          </div>
        </div>
        ...
        {!loading && !error && quizzes.length === 0 && (
          <div className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-slate-900 mb-1">Chưa có đề thi nào</h3>
            <p className="text-slate-500 mb-4 text-sm">Hãy bắt đầu tạo bài kiểm tra đầu tiên của bạn.</p>
            <Link href="/quizzes/create">
        ...
        {!loading && !error && quizzes.length > 0 && (
          <div className="divide-y divide-slate-100 px-4">
            {quizzes.map((quiz) => (
              <div key={quiz.id} className="py-5 flex flex-col sm:flex-row gap-6 items-start sm:items-center hover:bg-slate-50 group transition-colors rounded-lg px-2 -mx-2">
                ...
                <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto sm:justify-end">
                  ...
                  {quiz.status === 'draft' && (
                    <Button variant="outline" size="sm" className="gap-2 text-slate-600 w-full sm:w-auto">
                      <Edit2 className="w-4 h-4" /> Tiếp tục soạn
                    </Button>
                  )}
                  
                  <DropdownMenu>
                    <DropdownMenuTrigger className="h-9 w-9 p-0 flex items-center justify-center rounded-md border bg-white hover:bg-slate-100 text-slate-500 shrink-0">
